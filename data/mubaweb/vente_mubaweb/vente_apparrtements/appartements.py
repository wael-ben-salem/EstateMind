"""
SCRAPER MUBAWAB ULTIMATE - VERSION APPARTEMENTS
Scraper intelligent pour appartements avec détection automatique du nombre d'annonces
Régions: La Marsa, La Soukra, Le Kram, Ariana Ville, Hammamet, Nabeul, Hammam Sousse, Raoued, Akouda, Tunis
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_appartements_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabAppartementsUltimateScraper:
    """Scraper ULTIMATE pour appartements - Extraction complète"""
    ...
    def __init__(self, delay: float = 1.0, timeout: int = 30):
        ...
        self.property_type = 'apartment-sale'
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        ...
        if page == 1:
            return f"{base}/fr/st/{region_key}/appartements-a-vendre"
        else:
            return f"{base}/fr/st/{region_key}/appartements-a-vendre:p:{page}"
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                # Vérifier si c'est une annonce d'appartement
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('appartement' in text.lower() or 'DT' in text or 'TND' in text):
    ...
    def extract_price(self, text: str) -> Optional[float]:
        """Extrait le prix d'un appartement"""
        if not text:
            return None
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour appartements (20,000 à 10 millions)
                        if 20000 <= price <= 10000000:
                            return price
                except:
                    continue
    ...
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces d'appartements"""
        cards = []
        ...
        for tag, attrs in card_selectors:
            ...
            for elem in elements:
                # Vérifier que c'est une annonce d'appartement
                text = elem.get_text()
                if ('m²' in text or 'm2' in text or 'appartement' in text.lower()) and elem not in cards:
                    cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce d'appartement"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour un appartement
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce d'appartement"""
        try:
            # Valider d'abord
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('appartement' in para.lower() or 'm²' in para or 'prix' in para.lower()):
            ...
            # Vérifier que c'est bien un appartement
            ...
            logger.error(f"Erreur parsing carte appartement: {e}")
            return None
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'un appartement depuis sa page détaillée"""
        details = {
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les appartements
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_appartement', 'nombre_pieces', 'nombre_chambres', 'nombre_sdb',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING APPARTEMENTS")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏢 SCRAPER MUBAWAB ULTIMATE - APPARTEMENTS")
    ...
    print("="*80)
    print(f"   📍 Régions: La Marsa, La Soukra, Le Kram, Ariana Ville, Hammamet, Nabeul, Hammam Sousse, Raoued, Akouda, Tunis")
    print(f"   🏢 Type: Appartements à vendre")
    ...
    print(f"📍 Régions: {len(['La Marsa', 'La Soukra', 'Le Kram', 'Ariana Ville', 'Hammamet', 'Nabeul', 'Hammam Sousse', 'Raoued', 'Akouda', 'Tunis'])}")
    ...
    try:
        scraper = MubawabAppartementsUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_appartements_complet_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces d'appartements complètement scrapées")
            ...
            if listings:
                ...
                for item in df_preview:
                    ...
                    print(f"    {item['Ville']} | {item['Pièces']}p/{item['Chambres']}ch | {item['Surface']} | {item['Prix']}")
                    print()
            ...
            print("   Vérifiez le fichier log: mubawab_appartements_ultimate.log")
