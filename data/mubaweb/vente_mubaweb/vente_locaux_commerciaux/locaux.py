"""
SCRAPER MUBAWAB ULTIMATE - VERSION LOCAUX COMMERCIAUX À VENDRE
Scraper intelligent pour locaux commerciaux à vendre avec détection automatique
Région: La Marsa
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_locaux_commerciaux_vente_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabLocauxCommerciauxUltimateScraper:
    """Scraper ULTIMATE pour locaux commerciaux à vendre - Extraction complète"""
    ...
    def __init__(self, delay: float = 1.0, timeout: int = 30):
        ...
        # Région à scraper pour locaux commerciaux
        self.regions_config = {
            'La Marsa': 'la-marsa'
        }
        
        # Type de bien = Local commercial à vendre
        self.property_type = 'commercial-sale'
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        ...
        # Format d'URL basé sur l'exemple fourni: /fr/st/la-marsa/locaux-a-vendre
        if page == 1:
            return f"{base}/fr/st/{region_key}/locaux-a-vendre"
        else:
            return f"{base}/fr/st/{region_key}/locaux-a-vendre:p:{page}"
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            # Méthode 2: Chercher par structure spécifique locaux commerciaux
            cards = []
            
            # Chercher toutes les divs avec du contenu
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                # Vérifier si c'est une annonce de local commercial
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('local' in text.lower() or 'commerce' in text.lower() or 'bureau' in text.lower() or 'commercial' in text.lower()):
    ...
    def extract_price(self, text: str) -> Optional[float]:
        """Extrait le prix d'un local commercial à vendre"""
        if not text:
            return None
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour locaux commerciaux (50,000 à 10,000,000 DT)
    ...
    def extract_price_per_m2(self, text: str) -> Optional[float]:
        """Extrait le prix au m² pour les locaux commerciaux"""
        if not text:
            return None
    ...
    def extract_commercial_type(self, text: str) -> str:
        ...
        if 'bureau' in text_lower and 'commercial' in text_lower:
            ...
            return "Atelier"
        elif 'espace commercial' in text_lower:
            return "Espace commercial"
        elif 'local commercial' in text_lower:
            return "Local commercial"
        elif 'fond de commerce' in text_lower:
            return "Fond de commerce"
        elif 'espace de vente' in text_lower:
            return "Espace de vente"
        elif 'plateau' in text_lower:
            return "Plateau de bureau"
        elif 'double hauteur' in text_lower:
            return "Local double hauteur"
        elif 'mezzanine' in text_lower:
            return "Local avec mezzanine"
        else:
            return "Local commercial"
    
    def extract_commercial_features(self, text: str) -> List[str]:
        ...
        feature_mapping = {
            ...
            'hauteur sous plafond': 'Hauteur sous plafond',
            'vitrine': 'Vitrine',
            'façade': 'Façade commerciale',
            'enseigne': 'Droit d\'enseigne',
            'monte charge': 'Monte-charge',
            'quai de déchargement': 'Quai de déchargement',
            'système d\'alarme': 'Système d\'alarme',
            'vidéosurveillance': 'Vidéosurveillance',
            'droit de bail': 'Droit au bail',
            'passage piéton': 'Passage piéton important',
            'zone commerciale': 'Zone commerciale',
            'centre commercial': 'Centre commercial',
            'rue commerçante': 'Rue commerçante',
            'accès handicapé': 'Accès handicapés',
    ...
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces de locaux commerciaux à vendre"""
        cards = []
        ...
        # 2. Recherche par structure de carte spécifique aux locaux commerciaux
        card_selectors = [
        ...
        for tag, attrs in card_selectors:
            ...
            for elem in elements:
                # Vérifier que c'est une annonce de local commercial à vendre
                text = elem.get_text()
                if ('m²' in text or 'm2' in text) and ('local' in text.lower() or 'commerce' in text.lower() or 'bureau' in text.lower() or 'commercial' in text.lower()) and elem not in cards:
                    cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce de local commercial à vendre"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour un local commercial à vendre
            is_commercial = 'local' in text_lower or 'commerce' in text_lower or 'bureau' in text_lower or 'commercial' in text_lower
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce de local commercial à vendre"""
        try:
            # Valider d'abord
            ...
            # Caractéristiques spécifiques aux locaux commerciaux
            ...
            data['type_local'] = self.extract_commercial_type(all_text)
            ...
            commercial_features = self.extract_commercial_features(all_text)
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('local' in para.lower() or 'commerce' in para.lower() or 'm²' in para or 'prix' in para.lower()):
            ...
            # Vérifier que c'est bien un local commercial à vendre
            if data['surface'] == 0 and 'local' not in text_lower and 'commerce' not in text_lower:
                data['is_valid'] = False
                return None
            ...
            logger.error(f"Erreur parsing carte local commercial vente: {e}")
            return None
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'un local commercial à vendre depuis sa page détaillée"""
        details = {
        ...
        try:
            ...
            # Hauteur sous plafond (important pour locaux commerciaux)
            if 'hauteur' in full_text:
                match = re.search(r'hauteur\s*(?:sous\s*plafond)?\s*[:\-]?\s*(\d+[.,]?\d*)\s*m', full_text)
                if match:
                    details['informations_supplementaires']['hauteur_sous_plafond'] = match.group(1).replace(',', '.')
            
            # Mezzanine
            if 'mezzanine' in full_text:
                details['informations_supplementaires']['mezzanine'] = 'Oui'
            
            # Double hauteur
            if 'double hauteur' in full_text or 'hauteur double' in full_text:
                details['informations_supplementaires']['double_hauteur'] = 'Oui'
            ...
            # Façade commerciale
            if 'façade' in full_text:
                details['informations_supplementaires']['facade_commerciale'] = 'Oui'
            
            # Droit d'enseigne
            if 'enseigne' in full_text:
                details['informations_supplementaires']['droit_enseigne'] = 'Oui'
            
            # 6. Conditions de vente spécifiques aux locaux commerciaux
            ...
            # Droit au bail (pour fonds de commerce)
            if 'droit au bail' in full_text or 'droit de bail' in full_text:
                details['conditions_vente']['droit_au_bail'] = 'Oui'
            
            # Murs commerciaux
            if 'murs commerciaux' in full_text:
                details['conditions_vente']['murs_commerciaux'] = 'Oui'
            
            # Fond de commerce
            if 'fond de commerce' in full_text:
                details['conditions_vente']['fond_de_commerce'] = 'Oui'
                
                # Valeur du fonds
                match = re.search(r'fonds?\s*(?:de\s*commerce)?\s*[:\-]?\s*(\d[\d\s]*)\s*(?:dt|tnd)', full_text)
                if match:
                    details['conditions_vente']['valeur_fonds_commerce'] = match.group(1).replace(' ', '')
            
            # Date de construction
            if 'construction' in full_text:
                match = re.search(r'construc\w*\s*(\d{4})', full_text)
                if match:
                    details['conditions_vente']['annee_construction'] = match.group(1)
            ...
            # PDF Catalogue (spécifique aux locaux commerciaux)
            pdf_links = soup.find_all('a', href=re.compile(r'\.pdf$'))
            if pdf_links:
                details['contact_info']['catalogue_pdf'] = 'Disponible'
                for link in pdf_links[:3]:  # Limiter à 3 PDFs
                    href = link.get('href', '')
                    if href:
                        if 'catalogue_pdf_urls' not in details['contact_info']:
                            details['contact_info']['catalogue_pdf_urls'] = []
                        details['contact_info']['catalogue_pdf_urls'].append(href)
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les locaux commerciaux à vendre
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_local', 'type_transaction', 
                'prix', 'prix_text', 'prix_m2', 'surface', 'surface_text',
                'nombre_sdb', 'annee_construction', 'etage',
        ...
        try:
            data = {
                'metadata': {
                    ...
                    'property_type': 'locaux_commerciaux_vente',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING LOCAUX COMMERCIAUX")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏢 SCRAPER MUBAWAB ULTIMATE - LOCAUX COMMERCIAUX À VENDRE")
    ...
    print("="*80)
    print(f"   📍 Régions: {', '.join(['La Marsa'])}")
    print(f"   🏢 Type: Locaux commerciaux à vendre")
    ...
    try:
        scraper = MubawabLocauxCommerciauxUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_locaux_commerciaux_vente_complet_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces de locaux commerciaux à vendre complètement scrapées")
            ...
            if listings:
                ...
                for i, listing in enumerate(listings[:3]):
                    df_preview.append({
                        ...
                        'SDB': f"{listing.get('nombre_sdb', 0)}"
                    })
                
                for item in df_preview:
                    print(f"  • {item['Région']} - {item['Titre']}")
                    print(f"    {item['Ville']} | {item['Type']} | {item['Surface']} | {item['Prix']} | {item['SDB']} SDB")
                    print()
            ...
            print("   Vérifiez le fichier log: mubawab_locaux_commerciaux_vente_ultimate.log")
