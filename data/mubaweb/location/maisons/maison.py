"""
SCRAPER MUBAWAB ULTIMATE - VERSION LOCATION MAISONS
Scraper intelligent pour maisons à louer avec détection automatique du nombre d'annonces
Région: La Marsa
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_location_maisons_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabLocationMaisonsUltimateScraper:
    """Scraper ULTIMATE pour maisons à louer - Extraction complète"""
    ...
    def __init__(self, delay: float = 1.0, timeout: int = 30):
        ...
        # Région à scraper (maisons à La Marsa)
        self.regions_config = {
            'La Marsa': 'la-marsa'
        }
        
        # Type de bien = Maison à louer
        self.property_type = 'house-rent'
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        """Construit l'URL de recherche pour une région (MAISONS)"""
        base = "https://www.mubawab.tn"
        region_key = self.regions_config.get(region, region.lower().replace(' ', '-'))
        
        # Format d'URL basé sur l'exemple fourni: /fr/st/la-marsa/maisons-a-louer
        if page == 1:
            return f"{base}/fr/st/{region_key}/maisons-a-louer"
        else:
            return f"{base}/fr/st/{region_key}/maisons-a-louer:p:{page}"
    ...
    def detect_total_ads_and_pages(self, url: str) -> Tuple[int, int, int]:
        ...
        try:
            ...
            # 1. Chercher le nombre total de résultats (exactement comme dans le HTML fourni)
            total_ads = 0
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('maison' in text.lower() or 'villa' in text.lower() or 'louer' in text.lower() or 'DT' in text or 'TND' in text):
    ...
    def extract_price(self, text: str) -> Optional[float]:
        """Extrait le prix d'une maison à louer"""
        if not text:
            return None
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour maisons (200 à 50,000 DT/mois)
                        if 200 <= price <= 50000:
                            return price
                except:
                    continue
    ...
    def extract_house_type(self, text: str) -> str:
        ...
        if 'villa' in text_lower:
            ...
            return "Pavillon"
        elif 'fermette' in text_lower:
            return "Fermette"
        elif 'riad' in text_lower:
            return "Riad"
        elif 'chalet' in text_lower:
            return "Chalet"
        elif 'rez-de-chaussée' in text_lower or 'rdc' in text_lower:
            return "Rez-de-chaussée"
        elif 'étage' in text_lower:
            return "Maison d'étage"
        else:
            return "Maison"
    ...
    def extract_house_features(self, text: str) -> List[str]:
        ...
        feature_mapping = {
            ...
            'sauna': 'Sauna',
            'jacuzzi': 'Jacuzzi',
            'animaux autorisés': 'Animaux domestiques autorisés',
    ...
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces de maisons à louer"""
        cards = []
        ...
        if not cards:
            ...
            for tag, attrs in card_selectors:
                ...
                for elem in elements:
                    text = elem.get_text()
                    if ('m²' in text or 'm2' in text or 'maison' in text.lower() or 'villa' in text.lower()) and elem not in cards:
                        cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce de maison à louer"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour une maison à louer
            is_house = 'maison' in text_lower or 'villa' in text_lower or 'duplex' in text_lower or 'triplex' in text_lower
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce de maison à louer"""
        try:
            # Valider d'abord
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('maison' in para.lower() or 'm²' in para or 'loyer' in para.lower()):
            ...
            # Vérifier que c'est bien une maison à louer
            ...
            logger.error(f"Erreur parsing carte maison location: {e}")
            return None
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'une maison à louer depuis sa page détaillée"""
        details = {
        ...
        try:
            ...
            if 'vue sur mer' in full_text or 'vue mer' in full_text:
                details['informations_supplementaires']['vue_sur_mer'] = 'Oui'
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les maisons à louer
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_maison', 'type_location', 
                'loyer', 'loyer_text', 'loyer_m2', 'surface', 'surface_text',
                'nombre_pieces', 'nombre_chambres', 'nombre_sdb', 'etage',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING LOCATION MAISONS")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏡 SCRAPER MUBAWAB ULTIMATE - LOCATION MAISONS")
    ...
    try:
        scraper = MubawabLocationMaisonsUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_location_maisons_la_marsa_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces de maisons à louer complètement scrapées")
            ...
            if listings:
                for i, listing in enumerate(listings[:3]):
                    ...
                    print(f"     🏠 {listing.get('type_maison', '')} | 🛏️  {listing.get('nombre_chambres', 0)} chambres")
            ...
            print(f"   5. Appliquez des filtres pour analyser par quartier/type/prix")
            ...
            print("   Vérifiez le fichier log: mubawab_location_maisons_ultimate.log")
