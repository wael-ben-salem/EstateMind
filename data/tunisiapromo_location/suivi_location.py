#!/usr/bin/env python
"""
Script pour suivre la progression du scraping Tunisiapromo Location
"""
import json
import os
...
def suivre_location():
    print("\n" + "="*60)
    print="📊 SUIVEUR TUNISIAPROMO LOCATION"
    print("="*60)
    ...
    try:
        ...
        # Lire les annonces
        if os.path.exists('tunisiapromo_location.json'):
            with open('tunisiapromo_location.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
            ...
            for annonce in data:
                ...
                if annonce.get('type_location'):
                    types_location[annonce['type_location']] += 1
            ...
            print=f"\n   📅 Types de location:"
            for type_loc, count in types_location.most_common():
                print=f"      {type_loc}: {count}"
            ...
            if prix_list:
                print=f"\n   💰 Loyer moyen: {sum(prix_list)/len(prix_list):,.0f} DT"
