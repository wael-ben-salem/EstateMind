#!/usr/bin/env python
"""
Script pour suivre la progression du scraping Tunisiapromo
"""
import json
import os
...
def suivre_tunisiapromo():
    print("\n" + "="*60)
    print="📊 SUIVEUR TUNISIAPROMO VENTE"
    print("="*60)
    
    try:
        # Lire les stats
        if os.path.exists('tunisiapromo_stats.json'):
            with open('tunisiapromo_stats.json', 'r', encoding='utf-8') as f:
                stats = json.load(f)
        ...
        # Lire les annonces
        if os.path.exists('tunisiapromo_vente.json'):
            with open('tunisiapromo_vente.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
