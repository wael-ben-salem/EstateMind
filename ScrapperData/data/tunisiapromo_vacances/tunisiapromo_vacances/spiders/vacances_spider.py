...
from ..items import TunisiapromoVacancesItem

class VacancesSpider(scrapy.Spider):
    name = "vacances"
    allowed_domains = ["tunisiapromo.com"]
    start_urls = ["https://www.tunisiapromo.com/location-vacances"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.checkpoint_file = 'vacances_checkpoint.json'
        ...
        print(f"\n" + "="*60)
        print("📌 REPRISE INTELLIGENTE DU SCRAPING VACANCES")
        print("="*60)
    ...
    def parse(self, response):
        ...
        if current_page_num <= self.last_scraped_page:
            ...
            if current_page_num % 50 == 0:
                print(f"\n⏭️ PAGE VACANCES {current_page_num} déjà scrapée le {self.last_scraped_date}")
        ...
        if current_page_num % 10 == 0:
            print(f"\n📄 PAGE VACANCES {current_page_num}")
        ...
        if current_page_num % 10 == 0:
            print(f"   ✅ Locations vacances sur cette page: {len(annonces_valides)}")
        ...
        for annonce in annonces_valides:
            ...
            if url_rel:
                ...
                prix, unite = self.parse_prix_vacances(prix_text)
        ...
        if current_page_num % 10 == 0 and nouvelles_annonces_page > 0:
            print(f"   🆕 Nouvelles locations vacances sur cette page: {nouvelles_annonces_page}")
    ...
    def handle_pagination_error(self, failure):
        ...
        print(f"\n⚠️ ERREUR PAGE VACANCES {page_num} - Tentative {tentative} - Attente {delai}s")
        time.sleep(delai)
    ...
    def handle_detail_error(self, failure):
        ...
        print(f"   ⚠️ Erreur détail vacances {annonce_id} - Tentative {tentative} - Attente {delai}s")
        time.sleep(delai)
    ...
    def parse_detail(self, response):
        """Parse les détails d'une annonce de location vacances"""
        item = TunisiapromoVacancesItem()
        ...
        # Type d'offre
        item['type_offre'] = 'Location vacances'
        ...
        if item.get('description'):
            ...
            if 'mer' in desc_lower or 'plage' in desc_lower or 'bord de mer' in desc_lower:
                item['proximite_mer'] = 'Oui'
            else:
                item['proximite_mer'] = 'Non spécifié'
        
        # Période disponible
        self.detect_periode(item)
    ...
    def parse_prix_vacances(self, prix_text):
        """Parse le prix spécial vacances (ex: '50 DT /jour', '300 DT /semaine')"""
        if not prix_text:
            return None, None
        
        # Extraire le nombre
        match = re.search(r'([\d\s]+)', prix_text)
        if match:
            nombre = match.group(1).replace(' ', '')
            try:
                prix = float(nombre)
                # Détecter l'unité
                prix_lower = prix_text.lower()
                if '/semaine' in prix_lower or '/sem' in prix_lower:
                    return prix, '/semaine'
                elif '/mois' in prix_lower:
                    return prix, '/mois'
                elif '/nuit' in prix_lower:
                    return prix, '/nuit'
                elif '/jour' in prix_lower:
                    return prix, '/jour'
                else:
                    return prix, '/jour'  # Par défaut
            except:
                pass
        return None, None
    ...
    def detect_periode(self, item):
        """Détecte la période de disponibilité dans la description"""
        ...
        if 'janvier' in desc:
            item['periode_disponible'] = 'Janvier'
        elif 'février' in desc or 'fevrier' in desc:
            item['periode_disponible'] = 'Février'
        elif 'mars' in desc:
            item['periode_disponible'] = 'Mars'
        elif 'avril' in desc:
            item['periode_disponible'] = 'Avril'
        elif 'mai' in desc:
            item['periode_disponible'] = 'Mai'
        elif 'juin' in desc:
            item['periode_disponible'] = 'Juin'
        elif 'juillet' in desc:
            item['periode_disponible'] = 'Juillet'
        elif 'août' in desc or 'aout' in desc:
            item['periode_disponible'] = 'Août'
        elif 'septembre' in desc:
            item['periode_disponible'] = 'Septembre'
        elif 'octobre' in desc:
            item['periode_disponible'] = 'Octobre'
        elif 'novembre' in desc:
            item['periode_disponible'] = 'Novembre'
        elif 'décembre' in desc or 'decembre' in desc:
            item['periode_disponible'] = 'Décembre'
        elif 'vacances' in desc:
            item['periode_disponible'] = 'Vacances'
        elif 'été' in desc or 'ete' in desc:
            item['periode_disponible'] = 'Été'
        elif 'toute l\'année' in desc or 'année' in desc:
            item['periode_disponible'] = 'Toute l\'année'
        else:
            item['periode_disponible'] = 'Non spécifié'
    
    def print_final_report(self):
        print("\n" + "="*70)
        print("📊 RAPPORT FINAL - SCRAPING VACANCES INTELLIGENT")
        print("="*70)
