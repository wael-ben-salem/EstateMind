"""
Script de nettoyage pour Tunisia Promo - Colocation
Nettoie et normalise TOUTES les données des annonces de colocation
"""
...
def clean_type_annonce(type_annonce: Optional[str]) -> str:
    """Normalise le type d'annonce"""
    if not type_annonce:
        return "Offre"
    
    type_lower = type_annonce.lower().strip()
    
    for key, value in TYPES_ANNONCES_NORMALISEES.items():
...
def clean_type_logement(type_logement: Optional[str]) -> str:
    """Normalise le type de logement en colocation"""
    if not type_logement:
        return "Non spécifié"
    
    type_lower = type_logement.lower().strip()
    
    for key, value in TYPES_LOGEMENTS_NORMALISES.items():
        if key in type_lower:
            return value
...
def clean_genre(genre: Optional[str]) -> str:
    """Normalise le genre recherché"""
    if not genre or genre == "Non spécifié":
        return "Non spécifié"
    
    genre_lower = genre.lower().strip()
    
    for key, value in GENRES_NORMALISES.items():
...
def clean_profil(profil: Any) -> List[str]:
    """Nettoie la liste des profils recherchés"""
    if not profil:
        return []
    
    if isinstance(profil, str):
        items = [profil]
    elif isinstance(profil, list):
        items = profil
    else:
        return []
    ...
    for item in items:
        ...
        for key, value in PROFILS_NORMALISES.items():
...
def clean_charges(charges: Any) -> Dict[str, Any]:
    ...
    if charges_dict.get('charges_incluses'):
        ...
        details = charges_dict.get('details', [])
        if details:
...
def clean_annonce(annonce: Dict) -> Dict:
    """
    Nettoie complètement une annonce de colocation Tunisia Promo
    """
    cleaned = {}
    ...
    try:
        ...
        # ===== 3. TYPE D'ANNONCE =====
        cleaned['type_annonce'] = clean_type_annonce(annonce.get('type_annonce'))
        ...
        cleaned['quartier'] = safe_str(annonce.get('quartier'))
        ...
        # ===== 7. TYPE DE LOGEMENT =====
        cleaned['type_logement'] = clean_type_logement(annonce.get('type_logement'))
        
        # ===== 8. GENRE RECHERCHÉ =====
        cleaned['genre_recherche'] = clean_genre(annonce.get('genre_recherche'))
        
        # ===== 9. DESCRIPTION =====
        cleaned['description'] = clean_text(annonce.get('description'))
        
        # ===== 10. PROFIL RECHERCHÉ =====
        profil = annonce.get('profil_recherche')
        if profil:
            cleaned['profil_recherche'] = clean_profil(profil)
        
        # ===== 11. CARACTÉRISTIQUES DU BIEN =====
        ...
        cleaned['nombre_colocataires'] = clean_number(annonce.get('nombre_colocataires'))
...
def process_tunisiapromo_file_clean(input_path: str, output_path: Optional[str] = None):
    """Nettoie un fichier JSON d'annonces de colocation"""
    ...
    for i, annonce in enumerate(annonces):
        try:
            ...
            if cleaned.get('type_annonce') == 'Offre':
                stats['offres'] += 1
            elif cleaned.get('type_annonce') == 'Demande':
                stats['demandes'] += 1
            
            genre = cleaned.get('genre_recherche')
            if genre == 'Femme':
                stats['femmes'] += 1
            elif genre == 'Homme':
                stats['hommes'] += 1
    ...
    print("\n" + "="*60)
    print("📊 STATISTIQUES DE NETTOYAGE - TUNISIA PROMO COLOCATION")
    print("="*60)
    ...
    print(f"📤 Offres: {stats['offres']}")
    print(f"📥 Demandes: {stats['demandes']}")
    print(f"👩 Recherche Femme: {stats['femmes']}")
    print(f"👨 Recherche Homme: {stats['hommes']}")
    print(f"👥 Recherche Mixte: {stats['mixte']}")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description="Nettoyage des données Tunisia Promo Colocation")
    parser.add_argument('--file', '-f', default='tunisiapromo_colocation_extrait.json', help='Fichier JSON à nettoyer')
