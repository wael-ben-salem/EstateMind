...
from ..items import TunisiapromoLocationItem

class LocationSpider(scrapy.Spider):
    name = "location"
    allowed_domains = ["tunisiapromo.com"]
    start_urls = ["https://www.tunisiapromo.com/location"]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.checkpoint_file = 'location_checkpoint.json'
        ...
        print(f"\n" + "="*60)
        print("📌 REPRISE INTELLIGENTE DU SCRAPING LOCATION")
        print("="*60)
    ...
    def parse(self, response):
        ...
        if current_page_num <= self.last_scraped_page:
            ...
            if current_page_num % 50 == 0:
                print(f"\n⏭️ PAGE LOCATION {current_page_num} déjà scrapée le {self.last_scraped_date}")
        ...
        if current_page_num % 10 == 0:
            print(f"\n📄 PAGE LOCATION {current_page_num}")
        ...
        if current_page_num % 10 == 0:
            print(f"   ✅ Locations sur cette page: {len(annonces_valides)}")
        ...
        for annonce in annonces_valides:
            ...
            if url_rel:
                ...
                type_location = self.detect_type_location(titre, description)
        ...
        if current_page_num % 10 == 0 and nouvelles_annonces_page > 0:
            print(f"   🆕 Nouvelles locations sur cette page: {nouvelles_annonces_page}")
        ...
        if len(annonces_valides) == 0:
            ...
            if current_page_num > 5000:
                print(f"\n🛑 ARRÊT DE SÉCURITÉ : Page {current_page_num} > 5000")
                self.print_final_report()
                return
        else:
    ...
    def handle_pagination_error(self, failure):
        ...
        print(f"\n⚠️ ERREUR PAGE {page_num} - Tentative {tentative} - Attente {delai}s")
        time.sleep(delai)
    ...
    def handle_detail_error(self, failure):
        ...
        print(f"   ⚠️ Erreur détail {annonce_id} - Tentative {tentative} - Attente {delai}s")
        time.sleep(delai)
    ...
    def parse_detail(self, response):
        """Parse les détails d'une annonce de location"""
        item = TunisiapromoLocationItem()
    ...
    def detect_type_location(self, titre, description):
        texte = f"{titre} {description}".upper()
        if 'ANNÉE' in texte or 'ANNUELLE' in texte:
            return 'Annuelle'
        elif 'SAISON' in texte or 'VACANCES' in texte:
            return 'Saisonnière'
        elif 'MENSUEL' in texte or 'MOIS' in texte:
            return 'Mensuelle'
        else:
            return 'Non spécifié'
    ...
    def print_final_report(self):
        print("\n" + "="*70)
        print("📊 RAPPORT FINAL - SCRAPING LOCATION INTELLIGENT")
        print("="*70)
