"""
Script de nettoyage pour Location de Maisons
Nettoie et normalise TOUS les champs spécifiques aux maisons
"""
...
def clean_type_maison(type_maison: Any) -> str:
    ...
    type_str = str(type_maison).lower().strip()
...
def clean_type_piscine(type_piscine: Any) -> Optional[str]:
    """Normalise le type de piscine"""
    if not type_piscine:
        return None
    
    type_str = str(type_piscine).lower().strip()
    
    for key, value in PISCINE_NORMALISEE.items():
        if key in type_str:
            return value
    
    return str(type_piscine).title()


def clean_type_jardin(type_jardin: Any) -> Optional[str]:
    """Normalise le type de jardin"""
    if not type_jardin:
        return None
    
    type_str = str(type_jardin).lower().strip()
    
    for key, value in JARDIN_NORMALISE.items():
        if key in type_str:
            return value
    
    return str(type_jardin).title()


def clean_profil_locataire(profil: Any) -> Optional[str]:
    """Normalise le profil locataire"""
    if not profil:
        return None
    
    profil_str = str(profil).lower().strip()
    
    for key, value in PROFIL_LOCATAIRE_NORMALISE.items():
        if key in profil_str:
            return value
    
    return str(profil).title()


def clean_duree_location(duree: Any) -> Optional[str]:
...
def clean_listing(listing: Dict) -> Dict:
    """
    Nettoie complètement un listing de location de maison
    """
    cleaned = {}
    
    # ===== 1. CHAMPS SIMPLES =====
    simple_fields = ['id', 'titre', 'url', 'description_courte', 'description_complete',
                     'page_source', 'is_valid', 'type_location', 'batiment', 'residence',
                     'etage', 'duree_location', 'disponibilite', 'profil_locataire']
    ...
    # ===== 6. NOMBRES =====
    numeric_fields = ['nombre_pieces', 'nombre_chambres', 'nombre_sdb', 'nombre_niveaux',
    ...
    bool_fields = [
        'est_partagee', 'a_jardin', 'a_piscine', 'a_terrasse', 'a_balcon',
    ...
    # ===== 8. TYPES SPÉCIFIQUES =====
    cleaned['type_jardin'] = clean_type_jardin(listing.get('type_jardin'))
    cleaned['type_piscine'] = clean_type_piscine(listing.get('type_piscine'))
...
def clean_json_file(input_path: str, output_path: Optional[str] = None, validate: bool = False):
    """
    Nettoie un fichier JSON de location de maisons
    """
    ...
    for i, listing in enumerate(listings):
        try:
            ...
            if cleaned.get('type_maison') and cleaned['type_maison'] != listing.get('type_maison'):
                stats['types_maison_normalises'] += 1
            if cleaned.get('a_jardin'):
                stats['jardins_confirmes'] += 1
    ...
    print("\n" + "="*70)
    print("📊 STATISTIQUES DE NETTOYAGE - LOCATION DE MAISONS")
    print("="*70)
    ...
    print(f"✅ Types de maison normalisés: {stats['types_maison_normalises']}")
    print(f"✅ Jardins confirmés: {stats['jardins_confirmes']}")
...
def main():
    parser = argparse.ArgumentParser(description="Nettoyage des JSON de location de maisons")
