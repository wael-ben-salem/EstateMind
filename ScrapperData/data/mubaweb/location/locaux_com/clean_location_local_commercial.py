"""
Script de nettoyage pour Location de Locaux Commerciaux
...
TYPES_LOCAUX_NORMALISES = {
    ...
    "espace de vente": "Espace de vente",
    "open space": "Open Space",
    "espace bureautique": "Espace bureautique",
    "plateau": "Plateau"
}

EMPLACEMENTS_NORMALISES = {
    "artère principale": "Artère principale",
    "avenue principale": "Avenue principale",
...
def clean_type_local(type_local: Any) -> str:
    ...
    type_str = str(type_local).lower().strip()
...
def clean_emplacement(emplacement: Any) -> Optional[str]:
    """Normalise le type d'emplacement"""
    if not emplacement:
        return None
    
    emplacement_str = str(emplacement).lower().strip()
    
    for key, value in EMPLACEMENTS_NORMALISES.items():
        if key in emplacement_str:
            return value
    
    return str(emplacement).title()
...
def clean_etat_local(etat: Any) -> Optional[str]:
    """Normalise l'état du local"""
    if not etat:
        return None
    
    etat_str = str(etat).lower().strip()
    
    for key, value in ETAT_LOCAL_NORMALISE.items():
        if key in etat_str:
            return value
...
def clean_activites(activites: Any) -> List[str]:
    """Nettoie la liste des activités possibles"""
    if not activites:
        return []
    ...
    if isinstance(activites, list):
        ...
        if ',' in activites:
            items = [a.strip() for a in activites.split(',') if a.strip()]
        else:
            items = [activites.strip()]
    else:
        return []
...
def clean_caracteristiques(caracs: Any) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if key_clean in ['etat', 'état']:
            result['etat'] = clean_etat_local(value)
    ...
    try:
        ...
        for key, value in data.items():
            ...
            if 'formulaire' in key_clean:
                ...
                result['vendeur'] = clean_text(value)
...
def clean_listing(listing: Dict) -> Dict:
    ...
    simple_fields = ['id', 'titre', 'url', 'description_courte', 'description_complete',
                     'page_source', 'is_valid', 'type_location', 'batiment',
                     'residence', 'complexe', 'vendeur']
    ...
    surface_fields = ['surface_terrasse', 'surface_mezzanine']
    ...
    activites = listing.get('activites_possibles') or listing.get('activites_possibles_extrait')
    cleaned['activites_possibles'] = clean_activites(activites)
    
    # ===== 7. NOMBRES =====
    numeric_fields = ['nombre_sdb', 'nombre_vitrines', 'parking_nombre', 'hauteur_sous_plafond',
                      'largeur_facade', 'montant_charges', 'depot_garantie', 'droit_au_bail']
    for field in numeric_fields:
        val = listing.get(field)
        cleaned[field] = clean_number(val)
    
    # ===== 8. EMPLACEMENT =====
    cleaned['emplacement'] = clean_emplacement(listing.get('emplacement'))
    
    # ===== 9. CARACTÉRISTIQUES SPÉCIFIQUES =====
    bool_fields = [
        'est_gerance_libre', 'a_vitrine', 'a_mezzanine', 'a_terrasse',
        'a_parking', 'parking_clientele', 'a_livraison', 'a_enseigne',
        'a_hotte', 'a_extraction', 'a_reserve', 'acces_independant',
        'acces_24h', 'a_climatisation', 'a_chauffage', 'a_securite',
        'a_ascenseur', 'a_monte_charge', 'accessibilite_pmr', 'normes_erp',
        'charges_incluses'
    ]
    ...
    # ===== 13. AMENITIES COMMERCIAL =====
    cleaned['amenities_commercial'] = clean_equipements(listing.get('amenities_commercial', []))
    ...
    # ===== 21. ÉTAT =====
    etat = listing.get('etat_bien') or listing.get('caracs_etat_bien') or listing.get('etat_local')
    cleaned['etat_bien'] = clean_etat_local(etat)
...
def clean_json_file(input_path: str, output_path: Optional[str] = None, validate: bool = False):
    """
    Nettoie un fichier JSON de location de locaux commerciaux
    """
    ...
    for i, listing in enumerate(listings):
        try:
            ...
            if cleaned.get('activites_possibles') != listing.get('activites_possibles'):
                stats['activites_nettoyees'] += 1
            ...
            if cleaned.get('type_local') and cleaned['type_local'] != listing.get('type_local'):
                stats['types_local_normalises'] += 1
    ...
    print("\n" + "="*70)
    print("📊 STATISTIQUES DE NETTOYAGE - LOCAUX COMMERCIAUX (LOCATION)")
    print("="*70)
    ...
    print(f"✅ Activités nettoyées: {stats['activites_nettoyees']}")
    ...
    print(f"✅ Types de local normalisés: {stats['types_local_normalises']}")
    print("="*70)
...
def main():
    parser = argparse.ArgumentParser(description="Nettoyage des JSON de location de locaux commerciaux")
