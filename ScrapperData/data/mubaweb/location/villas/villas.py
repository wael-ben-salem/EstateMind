"""
SCRAPER MUBAWAB ULTIMATE - VERSION LOCATION VILLAS
Scraper intelligent pour villas et maisons de luxe à louer avec détection automatique
Régions: La Marsa, Carthage, Hammamet, La Soukra
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_location_villas_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabLocationVillasUltimateScraper:
    """Scraper ULTIMATE pour villas et maisons de luxe à louer - Extraction complète"""
    ...
    def __init__(self, delay: float = 1.0, timeout: int = 30):
        ...
        # Régions à scraper pour les villas (URL format basé sur l'exemple fourni)
        self.regions_config = {
            'La Marsa': 'la-marsa',
            'Carthage': 'carthage',
            'Hammamet': 'hammamet',
        ...
        # Type de bien = Villa à louer (basé sur l'URL exemple)
        self.property_type = 'villa-rent'
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        """Construit l'URL de recherche pour une région (VILLAS)"""
        base = "https://www.mubawab.tn"
        region_key = self.regions_config.get(region, region.lower().replace(' ', '-'))
        
        # Format d'URL basé sur l'exemple fourni: /fr/st/la-marsa/villas-et-maisons-de-luxe-a-louer
        if page == 1:
            return f"{base}/fr/st/{region_key}/villas-et-maisons-de-luxe-a-louer"
        else:
            return f"{base}/fr/st/{region_key}/villas-et-maisons-de-luxe-a-louer:p:{page}"
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            # Méthode 2: Chercher par structure spécifique villas
            cards = []
            
            # Chercher toutes les divs avec du contenu
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                # Vérifier si c'est une annonce de villa à louer
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('villa' in text.lower() or 'maison' in text.lower() or 'louer' in text.lower() or 'DT' in text or 'TND' in text):
    ...
    def extract_price(self, text: str) -> Optional[float]:
        """Extrait le prix d'une villa à louer"""
        if not text:
            return None
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour villas (1,000 à 100,000 DT/mois)
    ...
    def extract_price_per_m2(self, text: str) -> Optional[float]:
        """Extrait le prix au m² pour les villas"""
        if not text:
            return None
    ...
    def extract_villa_type(self, text: str) -> str:
        ...
        if 's3' in text_lower or '3 pièces' in text_lower:
            return "Villa S3"
        elif 's4' in text_lower or '4 pièces' in text_lower:
            return "Villa S4"
        elif 's5' in text_lower or '5 pièces' in text_lower:
            return "Villa S5"
        elif 's6' in text_lower or '6 pièces' in text_lower:
            return "Villa S6"
        elif 's+' in text_lower or 's7+' in text_lower:
            return "Villa S+"
        elif 'duplex' in text_lower:
            return "Villa Duplex"
        elif 'triplex' in text_lower:
            return "Villa Triplex"
        elif 'plain-pied' in text_lower or 'plain pied' in text_lower:
            return "Villa de plain-pied"
        elif 'maison' in text_lower and 'villa' not in text_lower:
            return "Maison"
        elif 'chalet' in text_lower:
            return "Chalet"
        elif 'ferme' in text_lower:
            return "Ferme"
        elif 'riad' in text_lower:
            return "Riad"
        elif 'luxe' in text_lower or 'prestige' in text_lower:
            return "Villa de luxe"
        elif 'traditionnelle' in text_lower:
            return "Villa traditionnelle"
        elif 'moderne' in text_lower:
            return "Villa moderne"
        elif 'contemporaine' in text_lower:
            return "Villa contemporaine"
        else:
            return "Villa"
    ...
    def extract_duration(self, text: str) -> str:
        ...
        if 'court terme' in text_lower or 'vacances' in text_lower or 'saison' in text_lower or 'été' in text_lower:
            return "Court terme"
        elif 'long terme' in text_lower or 'année' in text_lower or 'annuel' in text_lower:
            return "Long terme"
        elif 'mois' in text_lower:
            match = re.search(r'(\d+)\s*mois', text_lower)
            if match:
                return f"{match.group(1)} mois"
        elif 'jour' in text_lower or 'daily' in text_lower:
            return "À la journée"
        elif 'week-end' in text_lower or 'weekend' in text_lower:
            return "Week-end"
        
        return "Long terme"
    
    def extract_villa_features(self, text: str) -> List[str]:
        """Extrait les caractéristiques spécifiques aux villas"""
        features = []
        text_lower = text.lower()
        ...
        feature_mapping = {
            ...
            'terrain de sport': 'Terrain de sport'
        }
    ...
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces de villas à louer"""
        cards = []
        ...
        for tag, attrs in card_selectors:
            ...
            for elem in elements:
                # Vérifier que c'est une annonce de villa à louer
                text = elem.get_text()
                if (('m²' in text or 'm2' in text or 'villa' in text.lower() or 'maison' in text.lower() or 'chalet' in text.lower()) and 
                    ('louer' in text.lower() or 'location' in text.lower())) and elem not in cards:
                    cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce de villa à louer"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour une villa à louer
            is_villa = 'villa' in text_lower or 'maison' in text_lower or 'chalet' in text_lower or 's3' in text_lower or 's4' in text_lower or 's5' in text_lower
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce de villa à louer"""
        try:
            # Valider d'abord
            ...
            # Caractéristiques spécifiques aux villas
            ...
            # Surface du terrain
            terrain_patterns = [
                r'terrain\s*[:\-]?\s*(\d+(?:[,\s]\d+)*\.?\d*)\s*m[²2]',
                r'terrain\s*de\s*(\d+(?:[,\s]\d+)*\.?\d*)\s*m[²2]',
                r'parcelle\s*[:\-]?\s*(\d+(?:[,\s]\d+)*\.?\d*)\s*m[²2]'
            ]
            ...
            for pattern in terrain_patterns:
                ...
                if match:
                    try:
                        terrain_str = match.group(1).replace(' ', '').replace(',', '.')
                        data['surface_terrain'] = float(terrain_str)
                        break
                    except:
                        continue
            ...
            # Durée de location
            data['duree_location'] = self.extract_duration(all_text)
            
            # Nombre de pièces/chambres/sdb si pas encore trouvé
            ...
            # Nombre de niveaux
            if 'niveau' in text_lower or 'étage' in text_lower:
                match = re.search(r'(\d+)\s*niveau', text_lower)
                if match:
                    data['niveaux'] = int(match.group(1))
                else:
                    # Compter les mentions d'étages
                    if 'rez-de-chaussée' in text_lower:
                        data['niveaux'] = 1
                    if '1er étage' in text_lower or 'premier étage' in text_lower:
                        data['niveaux'] = max(data['niveaux'], 2)
                    if '2ème étage' in text_lower or 'deuxième étage' in text_lower:
                        data['niveaux'] = max(data['niveaux'], 3)
            ...
            if features_section:
                ...
                for feature in feature_divs:
                    ...
                    if text_span:
                        ...
                        elif 'icon-elevator' in icon_class:
                        ...
                        elif 'icon-furnished' in icon_class:
                            equip_list.append('Meublé')
                            data['meuble'] = True
                        elif 'icon-cellar' in icon_class:
                            equip_list.append('Cave/Cellier')
            
            # Chercher d'autres équipements spécifiques aux villas
            villa_features = self.extract_villa_features(all_text)
            equip_list.extend([feat for feat in villa_features if feat not in equip_list])
            
            data['equipements'] = equip_list
            data['amenities_villa'] = villa_features
            
            # Mettre à jour les flags booléens basés sur les features
            if 'Piscine' in villa_features:
                data['piscine'] = True
            if 'Jardin' in villa_features:
                data['jardin'] = True
            if 'Garage' in villa_features:
                data['garage'] = True
            if 'Parking' in villa_features:
                data['parking'] = True
            if 'Meublé' in villa_features:
                data['meuble'] = True
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('villa' in para.lower() or 'maison' in para.lower() or 'm²' in para or 'loyer' in para.lower()):
            ...
            # Vérifier que c'est bien une villa à louer
            if data['surface'] == 0 and 'villa' not in text_lower and 'maison' not in text_lower:
                data['is_valid'] = False
                return None
            ...
            logger.error(f"Erreur parsing carte villa location: {e}")
            return None
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'une villa à louer depuis sa page détaillée"""
        details = {
        ...
        try:
            ...
            # Surface du terrain
            if 'terrain' in full_text:
                match = re.search(r'terrain\s*[:\-]?\s*(\d+(?:[,\s]\d+)*\.?\d*)\s*m[²2]', full_text)
                if match:
                    details['informations_supplementaires']['surface_terrain'] = match.group(1).replace(' ', '')
            
            # Année de construction
            if 'construction' in full_text or 'construit' in full_text:
                match = re.search(r'(\d{4})\s*(?:année|construit|construction)', full_text)
                if match:
                    details['informations_supplementaires']['annee_construction'] = match.group(1)
            
            # Nombre de niveaux
            if 'niveau' in full_text or 'étage' in full_text:
                match = re.search(r'(\d+)\s*niveau', full_text)
                if match:
                    details['informations_supplementaires']['niveaux'] = match.group(1)
            ...
            # Garage
            if 'garage' in full_text:
                details['informations_supplementaires']['garage'] = 'Oui'
            
            # Parking
            if 'parking' in full_text:
                details['informations_supplementaires']['parking'] = 'Oui'
            
            # Meublé ou non
            if 'meublé' in full_text or 'furnished' in full_text:
                details['informations_supplementaires']['meuble'] = 'Oui'
            elif 'vide' in full_text:
            ...
            if 'chauffage' in full_text:
                if 'central' in full_text:
                    details['informations_supplementaires']['chauffage'] = 'Central'
                elif 'gaz' in full_text:
                    details['informations_supplementaires']['chauffage'] = 'Gaz'
                elif 'électrique' in full_text:
                    details['informations_supplementaires']['chauffage'] = 'Électrique'
            ...
            # Durée minimale
            if 'durée' in full_text:
                match = re.search(r'durée\s*[:\-]?\s*(\d+)\s*(?:mois|an)', full_text)
                if match:
                    details['conditions_location']['duree_minimale'] = match.group(1)
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les villas à louer
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_villa', 'type_location', 'duree_location', 
                'loyer', 'loyer_text', 'loyer_m2', 'surface', 'surface_text',
                'surface_terrain', 'nombre_pieces', 'nombre_chambres', 'nombre_sdb',
                'niveaux', 'annee_construction', 'jardin', 'piscine', 'garage',
                'parking', 'meuble', 'description_courte', 'description_complete',
                'equipements', 'amenities_villa', 'equipements_detaille', 'images',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING LOCATION VILLAS")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏡 SCRAPER MUBAWAB ULTIMATE - LOCATION VILLAS")
    ...
    print("="*80)
    print(f"   📍 Régions: {', '.join(['La Marsa', 'Carthage', 'Hammamet', 'La Soukra'])}")
    print(f"   🏡 Type: Villas et maisons de luxe à louer")
    ...
    print(f"📍 Régions: {len(['La Marsa', 'Carthage', 'Hammamet', 'La Soukra'])}")
    ...
    try:
        scraper = MubawabLocationVillasUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_location_villas_complet_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces de villas à louer complètement scrapées")
            ...
            if listings:
                ...
                for i, listing in enumerate(listings[:3]):
                    df_preview.append({
                        ...
                        'Terrain': f"{listing.get('surface_terrain', 0):,.0f} m²" if listing.get('surface_terrain', 0) > 0 else '',
                        'Loyer': f"{listing.get('loyer', 0):,.0f} TND/mois" if listing.get('loyer', 0) > 0 else listing.get('loyer_text', ''),
                        'Piscine': 'Oui' if listing.get('piscine', False) else 'Non',
                        'Jardin': 'Oui' if listing.get('jardin', False) else 'Non'
                    })
                
                for item in df_preview:
                    print(f"  • {item['Région']} - {item['Titre']}")
                    print(f"    {item['Type']} | {item['Chambres']}ch | {item['Surface']}{' | Terrain: ' + item['Terrain'] if item['Terrain'] else ''}")
                    print(f"    {item['Loyer']} | Piscine: {item['Piscine']} | Jardin: {item['Jardin']}")
                    print()
            ...
            print(f"   5. Appliquez des filtres pour analyser par région/type/caractéristiques")
            ...
            print("   Vérifiez le fichier log: mubawab_location_villas_ultimate.log")
