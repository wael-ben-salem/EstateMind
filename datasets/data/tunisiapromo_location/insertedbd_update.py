...
DB_NAME = "EstateMind"
COLLECTION_NAME = "tunisiepromo.locations"
FILE_NAME = "tunisiapromo_location_final.json"

print(f"🚀 Mise à jour quotidienne Locations Tunisie Promo - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
...
print(f"📊 {len(annonces):,} locations dans le fichier".replace(',', ' '))
...
maisons = collection.count_documents({"type_bien_normalise": "maison"})

print(f"\n✅ Total locations: {total:,}".replace(',', ' '))
print(f"   • Appartements: {appartements:,}".replace(',', ' '))
print(f"   • Maisons: {maisons:,}".replace(',', ' '))
