"""
SCRAPER MUBAWAB ULTIMATE - VERSION TERRAINS
Scraper intelligent pour terrains avec détection automatique du nombre d'annonces
Régions: Hammamet, La Soukra, La Marsa, Nabeul, Kélibia, Raoued
Structure identique au scraper appartements
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_terrains_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabTerrainsUltimateScraper:
    """Scraper ULTIMATE pour terrains - Même structure que appartements"""
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        """Construit l'URL de recherche pour une région"""
        base = "https://www.mubawab.tn"
        region_key = self.regions_config.get(region, region.lower().replace(' ', '-'))
        
        if page == 1:
            return f"{base}/fr/st/{region_key}/terrains-a-vendre"
        else:
            return f"{base}/fr/st/{region_key}/terrains-a-vendre:p:{page}"
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            # Méthode 2: Chercher par structure spécifique terrain
            cards = []
            
            # Chercher toutes les divs avec du contenu
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                # Vérifier si c'est une annonce de terrain
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('terrain' in text.lower() or 'DT' in text or 'TND' in text):
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour terrains (1000 à 100 millions)
    ...
    def extract_price_per_m2(self, text: str) -> Optional[float]:
        """Extrait le prix au m² spécifique pour terrains"""
        if not text:
            return None
    ...
    def extract_surface(self, text: str) -> Optional[float]:
        """Extrait la surface d'un terrain"""
        if not text:
            return None
    ...
    def extract_land_type(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['agricole', 'verger', 'oliviers', 'plantation']):
            return "Terrain Agricole"
        elif any(word in text_lower for word in ['constructible', 'urbain', 'lotissement']):
            return "Terrain Constructible"
        elif any(word in text_lower for word in ['promotion', 'lot de terrain']):
            return "Lot de Terrain"
        elif any(word in text_lower for word in ['bord de mer', 'plage', 'vue mer']):
            return "Terrain Vue Mer"
        elif any(word in text_lower for word in ['résidentiel', 'quartier résidentiel']):
            return "Terrain Résidentiel"
        elif any(word in text_lower for word in ['commercial', 'bureau', 'activité']):
    ...
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces de terrains"""
        cards = []
        ...
        for tag, attrs in card_selectors:
            ...
            for elem in elements:
                # Vérifier que c'est une annonce de terrain
                text = elem.get_text()
                if ('m²' in text or 'm2' in text or 'terrain' in text.lower()) and elem not in cards:
                    cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce de terrain"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour un terrain
            is_terrain = 'terrain' in text_lower
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce de terrain"""
        try:
            # Valider d'abord
            ...
            for tag, attrs in location_selectors:
                ...
                if location_elem:
                    ...
                    if tag == 'i':
                        ...
                        if parent:
                            location_text = parent.get_text(strip=True).replace('Hammamet, Hammamet', 'Hammamet')
                    else:
                        location_text = location_elem.get_text(strip=True).replace('Hammamet, Hammamet', 'Hammamet')
            ...
            # Type de terrain
            data['type_terrain'] = self.extract_land_type(all_text)
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('terrain' in para.lower() or 'm²' in para or 'prix' in para.lower()):
                        data['description_courte'] = self.clean_text(para[:300])
                        break
            
            # Vocation (depuis la description)
            if 'constructible' in text_lower:
                data['vocation'] = 'Constructible'
            elif 'agricole' in text_lower:
                data['vocation'] = 'Agricole'
            elif 'résidentiel' in text_lower:
                data['vocation'] = 'Résidentiel'
            elif 'commercial' in text_lower:
                data['vocation'] = 'Commercial'
            elif 'promotion' in text_lower:
                data['vocation'] = 'Promotion immobilière'
            
            # Situation juridique
            if 'titre' in text_lower or 'juridique' in text_lower:
                if 'bleu' in text_lower:
                    data['situation_juridique'] = 'Titre foncier (bleu)'
                elif 'blanc' in text_lower:
                    data['situation_juridique'] = 'Contrat (blanc)'
                elif 'individuel' in text_lower:
                    data['situation_juridique'] = 'Titre individuel'
                else:
                    data['situation_juridique'] = 'Situation juridique précisée'
            
            # Viabilisation
            if any(word in text_lower for word in ['eau', 'électricité', 'gaz', 'onas', 'viabilisé']):
                data['viabilisation'] = 'Viabilisé'
            
            # Équipements spécifiques aux terrains
            equip_list = []
            equip_mapping = {
                'clôturé': 'Clôturé',
                'eau': 'Eau disponible',
                'électricité': 'Électricité disponible',
                'gaz': 'Gaz disponible',
                'route goudronnée': 'Accès route goudronnée',
                'vue mer': 'Vue sur mer',
                'vue campagne': 'Vue campagne',
            ...
            # Vérifier que c'est bien un terrain
            if data['surface'] == 0 and 'terrain' not in text_lower:
                data['is_valid'] = False
                return None
            ...
            logger.error(f"Erreur parsing carte terrain: {e}")
            return None
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'un terrain depuis sa page"""
        details = {
        ...
        try:
            ...
            # Caractéristiques détaillées (spécifiques terrains)
            ...
            # Type précis
            if 'terrain agricole' in full_text or 'verger' in full_text:
                details['informations_supplementaires']['type_precis'] = 'Agricole'
            elif 'terrain constructible' in full_text:
                details['informations_supplementaires']['type_precis'] = 'Constructible'
            elif 'lotissement' in full_text:
                details['informations_supplementaires']['type_precis'] = 'Lotissement'
            
            # Situation juridique
            if 'titre foncier' in full_text or 'titre bleu' in full_text:
                details['informations_supplementaires']['situation_juridique'] = 'Titre foncier'
            elif 'contrat' in full_text or 'titre blanc' in full_text:
                details['informations_supplementaires']['situation_juridique'] = 'Contrat'
            
            # Viabilisation
            if any(word in full_text for word in ['eau disponible', 'électricité disponible', 'viabilisé']):
                details['informations_supplementaires']['viabilisation'] = 'Oui'
                        ...
                        try:
                            if listing.get('url'):
                                ...
                                listing.update({
                                    ...
                                    'images': '; '.join(details.get('images', [])[:10]),  # Limiter à 10 images
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les terrains
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_terrain', 'vocation', 'prix', 'prix_text', 'prix_m2',
                'surface', 'surface_text', 'situation_juridique', 'viabilisation',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING TERRAINS")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏞️  SCRAPER MUBAWAB ULTIMATE - TERRAINS")
    print("🎯 DÉTECTION AUTOMATIQUE - STRUCTURE IDENTIQUE AUX APPARTEMENTS")
    print("="*80)
    ...
    print("="*80)
    print(f"   📍 Régions: Hammamet, La Soukra, La Marsa, Nabeul, Kélibia, Raoued")
    print(f"   🏞️  Type: Terrains à vendre")
    ...
    print(f"📍 Régions: {len(['Hammamet', 'La Soukra', 'La Marsa', 'Nabeul', 'Kélibia', 'Raoued'])}")
    ...
    try:
        scraper = MubawabTerrainsUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_terrains_complet_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces de terrains complètement scrapées")
            ...
            if listings:
                ...
                for i, listing in enumerate(listings[:3]):
                    df_preview.append({
                        ...
                        'Prix/m²': f"{listing.get('prix_m2', 0):,.0f} DT" if listing.get('prix_m2', 0) > 0 else 'N/A'
                    })
                
                for item in df_preview:
                    print(f"  • {item['Région']} - {item['Titre']}")
                    print(f"    {item['Ville']} | {item['Prix']} | {item['Surface']} | {item['Prix/m²']}")
                    print()
            ...
            print(f"   5. Appliquez des filtres pour analyser par région/ville")
            ...
            print("   Vérifiez le fichier log: mubawab_terrains_ultimate.log")
