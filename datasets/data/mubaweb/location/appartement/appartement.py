"""
SCRAPER MUBAWAB ULTIMATE - VERSION LOCATION APPARTEMENTS
Scraper intelligent pour appartements à louer avec détection automatique du nombre d'annonces
Régions: La Marsa, Le Kram, La Soukra, Hammamet, Ariana Ville, Hammam Sousse, Nabeul, Carthage
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_location_appartements_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabLocationAppartementsUltimateScraper:
    """Scraper ULTIMATE pour appartements à louer - Extraction complète"""
    ...
    def __init__(self, delay: float = 1.0, timeout: int = 30):
        ...
        # Régions à scraper pour la location (URL format)
        self.regions_config = {
            'La Marsa': 'la-marsa',
            'Le Kram': 'le-kram',
            'La Soukra': 'la-soukra',
        ...
        # Type de bien = Appartement à louer
        self.property_type = 'apartment-rent'
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        """Construit l'URL de recherche pour une région (LOCATION)"""
        base = "https://www.mubawab.tn"
        region_key = self.regions_config.get(region, region.lower().replace(' ', '-'))
        
        if page == 1:
            return f"{base}/fr/st/{region_key}/appartements-a-louer"
        else:
            return f"{base}/fr/st/{region_key}/appartements-a-louer:p:{page}"
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            ...
            # Méthode 2: Chercher par structure spécifique appartement
            cards = []
            
            # Chercher toutes les divs avec du contenu
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                # Vérifier si c'est une annonce d'appartement à louer
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('appartement' in text.lower() or 'louer' in text.lower() or 'DT' in text or 'TND' in text):
    ...
    def extract_price(self, text: str) -> Optional[float]:
        """Extrait le prix d'un appartement à louer"""
        if not text:
            return None
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour location (100 à 10,000 DT/mois)
                        if 100 <= price <= 10000:
                            return price
                except:
                    continue
    ...
    def extract_price_per_m2(self, text: str) -> Optional[float]:
        """Extrait le prix au m² pour la location"""
        if not text:
            return None
    ...
    def extract_apartment_type(self, text: str) -> str:
        """Détermine le type d'appartement"""
        if not text:
            return "Appartement"
        
        text_lower = text.lower()
        
        if 's1' in text_lower or 'studio' in text_lower or '1 pièce' in text_lower:
            return "Studio/S1"
        elif 's2' in text_lower or '2 pièces' in text_lower:
            return "S2"
        elif 's3' in text_lower or '3 pièces' in text_lower:
            return "S3"
        elif 's4' in text_lower or '4 pièces' in text_lower:
            return "S4"
        elif 's+' in text_lower or 's5+' in text_lower or '5 pièces' in text_lower:
            return "S+"
        elif 'duplex' in text_lower:
            return "Duplex"
        elif 'triplex' in text_lower:
            return "Triplex"
        elif 'penthouse' in text_lower or 'attico' in text_lower:
            return "Penthouse"
        elif 'meublé' in text_lower or 'furnished' in text_lower:
            return "Appartement Meublé"
        elif 'vide' in text_lower:
            return "Appartement Vide"
        else:
            return "Appartement"
    ...
    def extract_duration(self, text: str) -> str:
        ...
        if 'court terme' in text_lower or 'vacances' in text_lower or 'saison' in text_lower:
            return "Court terme"
    ...
    def extract_charges_incluses(self, text: str) -> bool:
        """Détermine si les charges sont incluses"""
        if not text:
            return False
        
        text_lower = text.lower()
        
        if any(phrase in text_lower for phrase in ['charges comprises', 'charges incluses', 'frais inclus', 'tout compris']):
            return True
        elif any(phrase in text_lower for phrase in ['hors charges', 'sans charges', 'charges non comprises']):
            return False
        
        return False
    
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces d'appartements à louer"""
        cards = []
        ...
        for tag, attrs in card_selectors:
            ...
            for elem in elements:
                # Vérifier que c'est une annonce d'appartement à louer
                text = elem.get_text()
                if ('m²' in text or 'm2' in text or 'appartement' in text.lower() or 'louer' in text.lower()) and elem not in cards:
                    cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce d'appartement à louer"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour un appartement à louer
            is_appart = 'appartement' in text_lower or 's1' in text_lower or 's2' in text_lower or 's3' in text_lower
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce d'appartement à louer"""
        try:
            # Valider d'abord
            ...
            # Caractéristiques spécifiques aux appartements
            ...
            # Type d'appartement
            data['type_appartement'] = self.extract_apartment_type(all_text)
            
            # Durée de location
            data['duree_location'] = self.extract_duration(all_text)
            
            # Charges incluses
            data['charges_incluses'] = self.extract_charges_incluses(all_text)
            
            # Nombre de pièces si pas encore trouvé
            ...
            # Caution
            if 'caution' in text_lower:
                match = re.search(r'caution\s*[:\-]?\s*(\d[\d\s]*)\s*(?:dt|tnd)', text_lower)
                if match:
                    data['caution'] = match.group(1).replace(' ', '')
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('appartement' in para.lower() or 'm²' in para or 'loyer' in para.lower()):
            ...
            # Chercher d'autres équipements dans le texte
            equip_mapping = {
                'meublé': 'Meublé',
                'garage': 'Garage',
                'parking': 'Parking',
            ...
            for fr_key, fr_text in equip_mapping.items():
                if fr_key in text_lower and fr_text not in equip_list:
                    equip_list.append(fr_text)
            
            data['equipements'] = equip_list
            
            # Meublé ou non
            data['meuble'] = 'meublé' in text_lower
            ...
            # Vérifier que c'est bien un appartement à louer
            if data['surface'] == 0 and 'appartement' not in text_lower:
                data['is_valid'] = False
                return None
            ...
            logger.error(f"Erreur parsing carte appartement location: {e}")
            return None
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'un appartement à louer depuis sa page détaillée"""
        details = {
        ...
        try:
            ...
            # 3. Images complètes (limitées à 15 pour éviter les fichiers trop gros)
            img_tags = soup.find_all('img', src=re.compile(r'mubawab-media\.com'))
            for img in img_tags[:15]:  # Limiter à 15 images
                src = img.get('src')
            ...
            # 5. Informations supplémentaires depuis tout le texte
            ...
            if 'immeuble' in full_text or 'résidence' in full_text:
                match = re.search(r'immeuble.*?(\d+)\s*étage', full_text)
                if match:
                    details['informations_supplementaires']['etage_immeuble'] = match.group(1)
            ...
            # Garage/Parking
            if 'garage' in full_text or 'parking' in full_text:
            ...
            # Type de cuisine
            if 'cuisine équipée' in full_text or 'cuisine équipee' in full_text:
                details['informations_supplementaires']['cuisine'] = 'Équipée'
            elif 'cuisine aménagée' in full_text or 'cuisine amenagee' in full_text:
                details['informations_supplementaires']['cuisine'] = 'Aménagée'
            elif 'cuisine' in full_text:
                details['informations_supplementaires']['cuisine'] = 'Standard'
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les appartements à louer
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_appartement', 'type_location', 'duree_location', 
                'loyer', 'loyer_text', 'loyer_m2', 'charges_incluses', 'caution',
                'surface', 'surface_text', 'nombre_pieces', 'nombre_chambres', 'nombre_sdb',
                'etage', 'etage_immeuble', 'meuble', 'immeuble', 'residence',
                'description_courte', 'description_complete', 'equipements',
                'equipements_detaille', 'images', 'caracteristiques', 'localisation',
                'informations_supplementaires', 'conditions_location', 'contact_info',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING LOCATION APPARTEMENTS")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏢 SCRAPER MUBAWAB ULTIMATE - LOCATION APPARTEMENTS")
    ...
    print("="*80)
    print(f"   📍 Régions: La Marsa, Le Kram, La Soukra, Hammamet, Ariana Ville, Hammam Sousse, Nabeul, Carthage")
    print(f"   🏢 Type: Appartements à louer")
    ...
    print(f"📍 Régions: {len(['La Marsa', 'Le Kram', 'La Soukra', 'Hammamet', 'Ariana Ville', 'Hammam Sousse', 'Nabeul', 'Carthage'])}")
    ...
    try:
        scraper = MubawabLocationAppartementsUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_location_appartements_complet_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces d'appartements à louer complètement scrapées")
            ...
            if listings:
                ...
                for i, listing in enumerate(listings[:3]):
                    df_preview.append({
                        ...
                        'Pièces': f"{listing.get('nombre_pieces', 0)}",
                        ...
                        'Meublé': 'Oui' if listing.get('meuble', False) else 'Non'
                    })
                
                for item in df_preview:
                    print(f"  • {item['Région']} - {item['Titre']}")
                    print(f"    {item['Ville']} | {item['Pièces']}p/{item['Chambres']}ch | {item['Surface']} | {item['Loyer']} | {item['Meublé']}")
                    print()
            ...
            print("   Vérifiez le fichier log: mubawab_location_appartements_ultimate.log")
