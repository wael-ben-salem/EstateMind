"""
Script de nettoyage pour Location de Villas
...
def clean_listing(listing: Dict) -> Dict:
    """
    Nettoie complètement un listing de location de villa
    """
    cleaned = {}
    
    # ===== 1. CHAMPS SIMPLES =====
    simple_fields = ['id', 'titre', 'url', 'description_courte', 'description_complete',
                     'page_source', 'is_valid', 'type_location', 'duree_location',
                     'reference', 'disponibilite', 'proximite_mer']
    ...
    # ===== 6. NOMBRES =====
    numeric_fields = ['nombre_pieces', 'nombre_chambres', 'nombre_sdb', 'niveaux',
    ...
    bool_fields = [
        'jardin', 'piscine', 'garage', 'parking', 'meuble',
        'a_terrasse', 'a_chambre_service', 'a_dressing',
        'a_cheminee', 'a_ascenseur', 'a_securite',
        'a_spa', 'a_hammam', 'a_jacuzzi', 'a_climatisation',
        'a_chauffage', 'animaux_acceptes'
    ]
    ...
    amenities = listing.get('amenities_villa', [])
    ...
    # ===== 12. AMENITIES VILLA =====
    cleaned['amenities_villa'] = clean_equipements(listing.get('amenities_villa', []))
...
def clean_json_file(input_path: str, output_path: Optional[str] = None, validate: bool = False):
    """
    Nettoie un fichier JSON de location de villas
    """
    ...
    stats = {
        ...
        'images_filtrees': 0,
        'types_villa_normalises': 0,        
        'jardins_confirmees': 0,
    ...
    for i, listing in enumerate(listings):
        try:
            ...
            if cleaned.get('type_villa') and cleaned['type_villa'] != listing.get('type_villa'):
                stats['types_villa_normalises'] += 1
            if cleaned.get('jardin'):
                stats['jardins_confirmees'] += 1
            if cleaned.get('piscine'):
    ...
    print("\n" + "="*70)
    print("📊 STATISTIQUES DE NETTOYAGE - LOCATION DE VILLAS")
    print("="*70)
    ...
    print(f"✅ Types de villa normalisés: {stats['types_villa_normalises']}")
    print(f"✅ Jardins confirmés: {stats['jardins_confirmees']}")
...
def main():
    parser = argparse.ArgumentParser(description="Nettoyage des JSON de location de villas")
