"""
SCRAPER MUBAWAB ULTIMATE - VERSION LOCATION LOCAUX COMMERCIAUX
Scraper intelligent pour locaux commerciaux à louer avec détection automatique
Région: La Marsa
Structure complète avec extraction de toutes les données
Adapté à la structure exacte du site Mubawab
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mubawab_location_locaux_commerciaux_ultimate.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class MubawabLocationLocauxCommerciauxUltimateScraper:
    """Scraper ULTIMATE pour locaux commerciaux à louer - Extraction complète"""
    ...
    def __init__(self, delay: float = 1.0, timeout: int = 30):
        ...
        # Région à scraper (locaux commerciaux à La Marsa)
        self.regions_config = {
            'La Marsa': 'la-marsa'
        }
        
        # Type de bien = Local commercial à louer
        self.property_type = 'commercial-rent'
    ...
    def build_search_url(self, region: str, page: int = 1) -> str:
        """Construit l'URL de recherche pour une région (LOCAUX COMMERCIAUX)"""
        base = "https://www.mubawab.tn"
        region_key = self.regions_config.get(region, region.lower().replace(' ', '-'))
        
        # Format d'URL basé sur l'exemple fourni: /fr/st/la-marsa/locaux-a-louer
        if page == 1:
            return f"{base}/fr/st/{region_key}/locaux-a-louer"
        else:
            return f"{base}/fr/st/{region_key}/locaux-a-louer:p:{page}"
    ...
    def detect_total_ads_and_pages(self, url: str) -> Tuple[int, int, int]:
        ...
        try:
            ...
            # Méthode 1: Chercher l'élément avec id="numResults" (présent dans le HTML)
            num_results_elem = soup.find('span', id='numResults')
            if num_results_elem:
                result_text = num_results_elem.get_text(strip=True)
                match = re.search(r'(\d+)', result_text)
                if match:
                    total_ads = int(match.group(1))
                    logger.info(f"✅ Détecté via numResults: {total_ads} annonces")
            
            # Méthode 2: Chercher dans la classe "resultNum"
            if total_ads == 0:
                result_num_elem = soup.find('span', class_='resultNum')
                if result_num_elem:
                    result_text = result_num_elem.get_text(strip=True)
                    match = re.search(r'(\d+)', result_text)
                    if match:
                        total_ads = int(match.group(1))
                        logger.info(f"✅ Détecté via resultNum: {total_ads} annonces")
    ...
    def count_real_listings_on_page(self, soup: BeautifulSoup) -> int:
        ...
        try:
            # Chercher les conteneurs d'annonces (exactement comme dans le HTML)
            listing_boxes = soup.find_all('div', class_='listingBox')
            
            if listing_boxes:
                logger.debug(f"ListingBox trouvés: {len(listing_boxes)}")
                return len(listing_boxes)
            
            # Si pas de listingBox, chercher par d'autres méthodes
            cards = []
            for div in soup.find_all('div', class_=re.compile(r'col-\d+')):
                text = div.get_text()
                if ('m²' in text or 'm2' in text) and ('local' in text.lower() or 'commercial' in text.lower() or 'bureau' in text.lower() or 'louer' in text.lower() or 'DT' in text or 'TND' in text):
    ...
    def find_pagination(self, soup: BeautifulSoup):
        ...
        # Chercher la pagination dans la page
    ...
    def extract_price(self, text: str) -> Optional[float]:
        """Extrait le prix d'un local commercial à louer"""
        if not text:
            return None
        
        text = self.clean_text(text).lower()
        
        # Pour les locaux commerciaux, "Prix à consulter" est commun
        if any(phrase in text for phrase in ['à consulter', 'sur demande', 'prix sur demande']):
            return None
            ...
            if match:
                try:
                    ...
                    if price_str.isdigit():
                        ...
                        # Filtre de validité pour locaux commerciaux (100 à 100,000 DT/mois)
                        if 100 <= price <= 100000:
                            return price
                except:
                    continue
    ...
    def extract_surface(self, text: str) -> Optional[float]:
        """Extrait la surface d'un local commercial"""
        if not text:
            return None
    ...
    def extract_local_type(self, text: str) -> str:
        """Détermine le type de local commercial"""
        if not text:
        ...
        if 'open space' in text_lower or 'open-space' in text_lower:
            return "Open Space"
        elif 'bureau' in text_lower:
            return "Bureau"
        elif 'boutique' in text_lower:
            return "Boutique"
        elif 'magasin' in text_lower:
            return "Magasin"
            ...
            return "Atelier"
        elif 'restaurant' in text_lower or 'café' in text_lower or 'cafe' in text_lower:
            return "Restaurant/Café"
        elif 'pharmacie' in text_lower:
            return "Pharmacie"
        elif 'coiffure' in text_lower or 'esthétique' in text_lower:
            return "Salon de coiffure/Esthétique"
        elif 'clinique' in text_lower:
            return "Clinique"
        elif 'complexe' in text_lower:
            return "Complexe commercial"
        else:
            return "Local commercial"
    
    def extract_local_features(self, text: str) -> List[str]:
        """Extrait les caractéristiques spécifiques aux locaux commerciaux"""
        features = []
        text_lower = text.lower()
    ...
    def extract_commercial_activity(self, text: str) -> List[str]:
        """Extrait les activités commerciales possibles"""
        activities = []
        text_lower = text.lower()
        ...
        for fr_key, fr_text in activity_mapping.items():
            if fr_key in text_lower and fr_text not in activities:
                activities.append(fr_text)
        
        return activities
    
    def find_all_listing_cards(self, soup: BeautifulSoup) -> List:
        """Trouve TOUTES les cartes d'annonces de locaux commerciaux à louer"""
        cards = []
        
        # Structure principale Mubawab (exactement comme dans le HTML fourni)
        listing_boxes = soup.find_all('div', class_='listingBox')
        if listing_boxes:
        ...
        if not cards:
            ...
            for tag, attrs in card_selectors:
                ...
                for elem in elements:
                    text = elem.get_text()
                    if ('m²' in text or 'm2' in text or 'local' in text.lower() or 'commercial' in text.lower() or 'bureau' in text.lower()) and elem not in cards:
                        cards.append(elem)
    ...
    def validate_listing_card(self, card) -> bool:
        """Valide si une carte est une vraie annonce de local commercial à louer"""
        try:
            text = card.get_text()
            text_lower = text.lower()
            
            # Critères pour un local commercial à louer
            is_commercial = 'local' in text_lower or 'commercial' in text_lower or 'bureau' in text_lower or 'boutique' in text_lower or 'magasin' in text_lower
            has_surface = 'm²' in text or 'm2' in text
            has_price_marker = any(marker in text for marker in ['DT', 'TND', 'dinars', 'prix', 'loyer', 'à consulter'])
            is_for_rent = 'louer' in text_lower or 'location' in text_lower
            
            # Vérifier la structure (basé sur le HTML fourni)
            ...
            # Score de validation
            score = 0
            if is_commercial or has_surface:
                score += 2
            if has_price_marker:
                score += 1
            if is_for_rent:
    ...
    def parse_listing_card(self, card) -> Optional[Dict]:
        """Parse une carte d'annonce de local commercial à louer"""
        try:
            # Valider d'abord
            ...
            # URL et ID (basé sur la structure exacte du HTML)
            ...
            for link in links:
                ...
                if '/a/' in href or '/pa/' in href:
                    ...
                    # Titre depuis le lien (structure: h2.listingTit > a)
            ...
            # Prix (loyer) - structure: span.priceTag
            price_selectors = [
                ('span', {'class': 'priceTag'}),
                ('div', {'class': 'priceBar'}),
            ...
            # Localisation - structure: span.listingH3 avec icon-location
            location_selectors = [
            ...
            for tag, attrs in location_selectors:
                ...
                if location_elem:
                    ...
                    if location_text:
                        ...
                        # Extraire ville et quartier (format: "Quartier, Ville")
                        if ',' in location_text:
                            parts = location_text.split(',')
                            if len(parts) > 1:
                                data['quartier'] = parts[0].replace('à', '').strip()
            ...
            # Caractéristiques spécifiques - structure: div.adDetails
            ...
            # Type de local
            data['type_local'] = self.extract_local_type(all_text)
            
            # Activités possibles
            data['activites_possibles'] = self.extract_commercial_activity(all_text)
            
            # Équipements/Features - structure: div.adFeatures
            equip_list = []
            ...
            if features_section:
                ...
                for feature in feature_divs:
                    ...
                    if text_span:
                        ...
                        icon_mapping = {
                            'icon-elevator': 'Ascenseur',
                            'icon-garage': 'Garage',
                            'icon-terrace': 'Terrasse',
                            'icon-airConditioning': 'Climatisation',
                            'icon-heating': 'Chauffage',
                            'icon-fullKitchen': 'Cuisine équipée',
                            'icon-security': 'Sécurité',
                            'icon-doorman': 'Concierge',
                            'icon-doubleGlazing': 'Double vitrage',
                            'icon-storageRoom': 'Chambre rangement',
                            'icon-cellar': 'Cave/Entre-seul',
                            'icon-reinforcedDoor': 'Porte blindée',
                            'icon-satellite': 'Antenne parabolique',
                            'icon-internet': 'Internet'
                        }
                        if icon_class in icon_mapping:
                            equip_list.append(icon_mapping[icon_class])
            
            # Chercher d'autres équipements spécifiques aux locaux commerciaux
            commercial_features = self.extract_local_features(all_text)
            equip_list.extend([feat for feat in commercial_features if feat not in equip_list])
            
            data['equipements'] = equip_list
            data['amenities_commercial'] = commercial_features
            ...
            if not data['description_courte']:
                ...
                for para in paragraphs:
                    if len(para) > 30 and ('local' in para.lower() or 'commercial' in para.lower() or 'm²' in para or 'loyer' in para.lower()):
            ...
            if 'étage' in text_lower:
                ...
                elif 'rdc' in text_lower or 'rez-de-chaussée' in text_lower:
                    data['etage'] = 'RDC'
            
            # Complexe/Résidence
            if 'complexe' in text_lower:
                match = re.search(r'complexe\s+([\w\s\-]+)', text_lower)
                if match:
                    data['complexe'] = match.group(1).strip().title()
            ...
            # Vérifier que c'est bien un local commercial à louer
            if data['surface'] == 0 and 'local' not in text_lower and 'commercial' not in text_lower and 'bureau' not in text_lower:
                data['is_valid'] = False
                return None
            ...
            logger.error(f"Erreur parsing carte local commercial: {e}")
            return None
    ...
    def scrape_page_listings(self, url: str, page_num: int) -> Tuple[List[Dict], bool, Optional[str], int]:
        ...
        try:
            ...
            # Chercher le lien "suivant" ou les numéros de page
            ...
            if next_link and next_link.get('href'):
                ...
                has_next = True
                logger.info(f"↪️  Lien suivant trouvé: {next_url}")
            else:
                # Construire l'URL suivante
                if ':p:' in url:
    ...
    def scrape_detailed_listing(self, url: str) -> Dict:
        """Scrape les détails complets d'un local commercial à louer depuis sa page détaillée"""
        details = {
        ...
        try:
            ...
            for img in img_tags[:30]:  # Limité à 30 images
                src = img.get('src')
            ...
            if 'étage' in full_text:
                ...
                elif 'rdc' in full_text or 'rez-de-chaussée' in full_text:
                    details['informations_supplementaires']['etage'] = 'RDC'
            ...
            # Vitrine
            if 'vitrine' in full_text:
                details['informations_supplementaires']['vitrine'] = 'Oui'
            ...
            # Double vitrage
            if 'double vitrage' in full_text:
                details['informations_supplementaires']['double_vitrage'] = 'Oui'
            ...
            # Type de bail
            if 'bail commercial' in full_text:
                details['conditions_location']['type_bail'] = 'Bail commercial'
            elif 'bail professionnel' in full_text:
                details['conditions_location']['type_bail'] = 'Bail professionnel'
            ...
            # 8. Date de publication (dans les scripts JSON-LD)
            ...
            for script in script_tags:
                try:
                    ...
                    if 'seller' in json_data and 'name' in json_data['seller']:
                        details['contact_info']['vendeur'] = json_data['seller']['name']
                except:
                    pass
    ...
    def save_to_csv(self, listings: List[Dict], filename: str):
        ...
        try:
            ...
            # Ordre logique pour les locaux commerciaux à louer
            preferred_order = [
                'id', 'titre', 'url', 'region', 'ville', 'quartier', 'adresse',
                'type_local', 'type_location', 'activites_possibles',
                'loyer', 'loyer_text', 'loyer_m2', 'surface', 'surface_text',
                'nombre_sdb', 'etage', 'batiment', 'residence', 'complexe',
                'description_courte', 'description_complete',
                'equipements', 'amenities_commercial', 'equipements_detaille', 'images',
    ...
    def display_final_stats(self):
        ...
        print("\n" + "="*80)
        print("📊 STATISTIQUES FINALES DU SCRAPING LOCATION LOCAUX COMMERCIAUX")
        print("="*80)
...
def main():
    """Programme principal"""
    print("\n" + "="*80)
    print("🏢 SCRAPER MUBAWAB ULTIMATE - LOCATION LOCAUX COMMERCIAUX")
    print("🎯 LA MARSA - EXTRACTION COMPLÈTE DES DONNÉES")
    print("="*80)
    ...
    print("="*80)
    print(f"   📍 Région: La Marsa")
    print(f"   🏢 Type: Locaux commerciaux à louer")
    ...
    try:
        scraper = MubawabLocationLocauxCommerciauxUltimateScraper(delay=0.8)
        ...
        if listings:
            ...
            base_name = f"mubawab_location_locaux_commerciaux_la_marsa_{timestamp}"
            ...
            print(f"\n✅ SCRAPING RÉUSSI!")
            print(f"   {len(listings)} annonces de locaux commerciaux à louer complètement scrapées")
            ...
            if listings:
                for i, listing in enumerate(listings[:3]):
                    ...
                    print(f"     💰 {listing.get('loyer', 0):,.0f} TND/mois | 📐 {listing.get('surface', 0):,.0f} m²")
                    print(f"     🏢 {listing.get('type_local', '')}")
                    if listing.get('activites_possibles'):
                        print(f"     🛒 Activités: {', '.join(listing['activites_possibles'][:3])}")
                    print(f"     📍 {listing.get('quartier', '')}, {listing.get('ville', '')}")
                    print()
            ...
            print(f"   5. Appliquez des filtres pour analyser par type/activité/prix")
            ...
            print("   Vérifiez le fichier log: mubawab_location_locaux_commerciaux_ultimate.log")
