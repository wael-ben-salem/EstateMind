#!/usr/bin/env python
"""
Script pour suivre la progression du scraping Tunisiapromo Colocation
"""
import json
import os
...
def suivre_colocation():
    print("\n" + "="*60)
    print="📊 SUIVEUR TUNISIAPROMO COLOCATION"
    print("="*60)
    
    try:
        # Lire les stats
        if os.path.exists('colocation_stats.json'):
            with open('colocation_stats.json', 'r', encoding='utf-8') as f:
                stats = json.load(f)
        ...
        # Lire les annonces
        if os.path.exists('tunisiapromo_colocation.json'):
            with open('tunisiapromo_colocation.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            print(f"\n🏠 ANNONCES COLOCATION ({len(data)}):")
            ...
            villes = Counter()
            annonceurs = Counter()
            types_annonce = Counter()  # Offre/Demande
            ...
            for annonce in data:
                ...
                if annonce.get('type_annonce'):
                    types_annonce[annonce['type_annonce']] += 1
                if annonce.get('prix'):
            ...
            print=f"\n   📋 Types d'annonces:"
            for type_a, count in types_annonce.most_common():
                print=f"      {type_a}: {count}"
